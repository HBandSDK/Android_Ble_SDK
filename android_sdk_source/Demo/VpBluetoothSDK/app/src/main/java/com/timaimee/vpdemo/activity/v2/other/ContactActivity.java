...
public class ContactActivity extends BaseVPBLETestActivity implements IContactOptListener, OnRecycleViewClickCallback {
    ...
    public void initEvent() {
        ...
        findViewById(R.id.btnSOSSettings).setOnClickListener(v -> {
            Intent intent = new Intent(ContactActivity.this, AddContactActivity.class);
            intent.putExtra("_isAdd", false);
            startActivityForResult(intent, 16);
        });
        findViewById(R.id.btnAddAll).setOnClickListener(v -> {
            if (!isHasRead) {
            ...
            if (canAddSize > 0) {
                ...
                VPOperateManager.getInstance().addContactList(canAddList, ContactActivity.this, defaultResponse);
            } else {
        ...
        findViewById(R.id.tvRefresh).setOnClickListener(v -> {
            ToastUtil.show("刷新");
            VPOperateManager.getInstance().readContact(-1, ContactActivity.this, defaultResponse);
        });
        findViewById(R.id.tvDeleteAll).setOnClickListener(v -> {
            ...
            ToastUtil.show("删除全部");
            VPOperateManager.getInstance().deleteContactList(mListData, ContactActivity.this, defaultResponse);
        });
    }

    private void initListData() {
        VPOperateManager.getInstance().readContact(-1, this, defaultResponse);
    }
...
//                saveContact();
                ...
                if (fromContact != null && toContact != null) {
                    VPOperateManager.getInstance().moveContact(fromContact, toContact, ContactActivity.this, defaultResponse);
...
//        VPOperateManager.getInstance().readContact(-1, this, response);
    ...
    public void OnRecycleViewClick(int position) {
        ...
        if (position >= 0x10) {
            ...
            VPOperateManager.getInstance().setContactSOSState(contact.isSettingSOS(), contact, this, defaultResponse);
        } else {
            ...
            VPOperateManager.getInstance().deleteContact(contact, this, defaultResponse);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 15 && resultCode == 1115) {
            ...
            mAdapter.notifyDataSetChanged();
            VPOperateManager.getInstance().addContact(contact, this, defaultResponse);
        }
    }
}
