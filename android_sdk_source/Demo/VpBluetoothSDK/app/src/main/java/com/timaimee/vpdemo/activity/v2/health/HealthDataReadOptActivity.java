...
public class HealthDataReadOptActivity extends BaseActivity implements IOriginData3Listener {
    ...
    private List<CollapseCardView> cardList;
    ...
    public int getLayoutID() {
        return R.layout.activity_health_data_read_opt;
    }
    ...
    public void initView() {
        ...
        // 初始化折叠卡片
        ccvReadAll = findViewById(R.id.ccvReadAll);
        ccvReadCustom = findViewById(R.id.ccvReadCustom);
        ccvReadFrom = findViewById(R.id.ccvReadFrom);
        ccvReadOneDay = findViewById(R.id.ccvReadOneDay);
        cardList = Arrays.asList(ccvReadAll, ccvReadCustom, ccvReadFrom, ccvReadOneDay);
    ...
    public void initEvent() {
        for (int i = 0; i < cardList.size(); i++) {
            int finalI = i;
            cardList.get(i).setOnClickListener(v -> {
                for (int j = 0; j < cardList.size(); j++) {
                    if (j != finalI) cardList.get(j).collapse();
                }
                cardList.get(finalI).toggle();
            });
        }
